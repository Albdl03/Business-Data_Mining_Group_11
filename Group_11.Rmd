...
*   Decision Tree
*   Random Forest
*   Logistic Regression (not sure about that, at least to chose some variables and not all ones)
...
nn_model <- neuralnet(y_train ~ ., data = data.frame(x_train_norm, y_train), hidden = 4, linear.output = TRUE)
...
nn_model <- neuralnet(y_train ~ ., data = data.frame(x_train_norm, y_train), hidden = c(4, 4), stepmax = 1e6, linear.output = TRUE)
